...
class _ChatMoreViewState extends State<ChatMoreView> {
  ...
  void initState() {
    ...
    for (var i = 0; i < _pageNum; i++) {
      int count = widget.list.length - (i + 1) * int.parse(pageItemCount.toString());
      List<ChatMoreViewItem> list;
      if (count >= 0) {
        list = widget.list.sublist(i * int.parse(pageItemCount.toString()), int.parse(pageItemCount.toString()));
      } else {
        list = widget.list.sublist(i * int.parse(pageItemCount.toString()));
      }
